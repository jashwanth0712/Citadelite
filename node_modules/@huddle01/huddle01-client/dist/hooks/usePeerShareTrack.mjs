...
// src/hooks/usePeerShareTrack.ts
import { useState } from "react";
var usePeerMicTrack = (peerId) => {
  const [error, setError] = useState(null);
  const peerShareStream = useHuddleStore(
    (state) => {
      var _a;
      return (_a = state.peers[peerId].consumers) == null ? void 0 : _a.share;
    }
  );
  if (!peerShareStream) {
  ...
  const { track, appData, id, isPaused } = peerShareStream;
  const data = {
    id,
    isPaused,
    appData
...
var usePeerShareTrack_default = usePeerMicTrack;
export {
  usePeerShareTrack_default as default
};
