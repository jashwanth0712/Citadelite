...
// src/hooks/useMeShareAudioTrack.ts
import { useState } from "react";
var useMeShareAudioTrack = () => {
  const [error, setError] = useState(null);
  const meShareAudioStream = useHuddleStore(
    (state) => state.producers.shareAudio
  );
  if (!meShareAudioStream) {
    setError("No stream found");
    return {
      error
    };
  }
  const { track, appData, id, isPaused } = meShareAudioStream;
  const data = {
    id,
    isPaused,
    appData
...
var useMeShareAudioTrack_default = useMeShareAudioTrack;
export {
  useMeShareAudioTrack_default as default
};
