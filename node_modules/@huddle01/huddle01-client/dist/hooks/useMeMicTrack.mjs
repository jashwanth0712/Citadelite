...
// src/hooks/useMeMicTrack.ts
import { useState } from "react";
var useMeMicTrack = () => {
  const [error, setError] = useState(null);
  const meMicStream = useHuddleStore((state) => state.producers.mic);
  if (!meMicStream) {
  ...
  const { track, appData, id, isPaused } = meMicStream;
  const data = {
    id,
    isPaused,
    appData
...
var useMeMicTrack_default = useMeMicTrack;
export {
  useMeMicTrack_default as default
};
